/* Lógica principal con correcciones de usabilidad */
class GasolinerasApp {
  ...
  vincularEventos() {
    ...
    /* Slider radio - CORRECCIÓN: actualización automática */
  ...
  async reverseGeocode() {
    ...
    try {
      ...
      if (js.display_name) {
        ...
        // CORRECCIÓN: Poner la dirección en el campo de texto
  ...
  procesar(arr) {
    ...
    const lista = arr.map(g => {
      ...
      // CORRECCIÓN: Ordenamiento mejorado - primero por precio, luego por distancia
      .sort((a, b) => {
        if (Math.abs(a.precio - b.precio) < 0.001) { // Mismo precio (diferencia < 0.001€)
  ...
  marcar(lista) {
    ...
    lista.forEach((g, i) => {
      ...
      // CORRECCIÓN: Una sola caja con 2 líneas
